...
onBuy(page, item, price, index)
{
    ...
    buildWeapon = self.weaponData[BUILD_NAME];
    hasWeapon = player hasWeapon(buildWeapon);
    if (hasWeapon) player player_save_ammo(buildWeapon, "selected");

    player takeWeapon(buildWeapon);
    ...
    if (hasWeapon) player player_restore_ammo(buildWeapon, "selected");

    self.weaponData[BUILD_NAME] = buildWeapon;
    player setWeaponData(buildWeapon, self.weaponData);
    player lethalbeats\survival\utility::survivor_switch_to_weapon(buildWeapon);
