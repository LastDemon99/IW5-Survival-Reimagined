...
{	
	...
	self playerLinkTo(mi17, "rear_wheel");
...
_juggerDrop(mi17, rope, sp_jugger)
{
	...
	ropeOrigin = mi17 getTagOrigin("tag_fastrope_ri");
	...
	for (i = 0; i < 335; i++)
	{
		...
		if (i == 230) mi17 thread maps\mp\killstreaks\_helicopter::heli_leave();
		if (i == 300) rope unLink();
		if (i == 60)
		{
			forward = anglesToForward(sp_jugger.angles);
			sp_jugger unlink();
			sp_jugger moveTo(rope getTagOrigin("ropejoint10_ri") + (forward * 35), 0.5);
			sp_jugger.angles = rope getTagAngles("ropejoint10_ri");
			rope unlink();
		}
		if (i == 67) thread _juggerRopeDrop(sp_jugger, rope, mi17);
		if (i == 115)
		{
			self show();
			self unLink();
...
_juggerRopeDrop(sp_jugger, rope, mi17)
{
	...
	for (i = 11; i < 75; i++)
	{
		...
		ropePosition = rope getTagOrigin(jointTag);
		travelTime = distance(sp_jugger.origin, ropePosition) / 260;
		sp_jugger moveTo(ropePosition, travelTime);
		sp_jugger.angles = rope getTagAngles(jointTag);
		waittillframeend;
	}
}
...
_mi17_handleDamage()
{
	...
	while(true)
	{
		...
		{			
			...
			if(isDefined(self.owner) && !self.owner.isDropped)
