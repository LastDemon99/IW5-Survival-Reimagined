...
giveAbility()
{
	...
	dog.isIdle = false;
	dog.isRunning = false;
	dog.isRunningLeanRight = false;
	dog.isRunningLeanLeft = false;
...
onDogDeath()
{
	...
	dog_model.icon delete();
	self.dog = undefined;

	wait 0.25;
	
	dog_model scriptModelPlayAnim(DOG_PREFIX + DEATH);
...
updateAnimation()
{
	...
	for(;;)
	{
		...
		{
			case IDLE:
				if (!self.dog.isIdle) self setDogAnim(RUN_START, 0.3);
				break;
			case RUN_START:
				self setDogAnim(RUNNING);
				break;
			case RUNNING:
			case RUN_LEAN_L:
			case RUN_LEAN_R:
				if (self.dog.isIdle) self setDogAnim(RUN_STOP, 0.6);
				else if (self.dog.isRunningLeanLeft) self setDogAnim(RUN_LEAN_L);
				else if (self.dog.isRunningLeanRight) self setDogAnim(RUN_LEAN_R);
				else self setDogAnim(RUNNING);
				break;
			case RUN_STOP:
...
monitorTurn()
{
    ...
    for (;;) 
	{
        self.dog.isRunningLeanLeft = false;
        self.dog.isRunningLeanRight = false;
        ...
        if (angleDifference > 10 && angleDifference <= 45) self.dog.isRunningLeanRight = true;
        else if (angleDifference < -10 && angleDifference >= -45) self.dog.isRunningLeanLeft = true;
        wait 0.5;
    }
}
