...
sentrySetPlaced()
{
	...
	trigger = maps\mp\survival\_trigger::createTrigger("sentry_move", self.origin, 0, 55, 55, &"SENTRY_MOVE");
	trigger.sentry = self;
	...
	self.owner maps\mp\survival\_trigger::clearCustomHintString();
}
	...
	for(;;)
	{
		...
		if (sentryGun.canBePlaced != lastCanPlaceSentry)
		{
			if (sentryGun.canBePlaced)
			{
				...
				self maps\mp\survival\_trigger::setCustomHintString(&"SENTRY_PLACE");
			}
			else
			{
				sentryGun setModel(level.sentrySettings[sentryGun.sentryType].modelPlacementFailed);
				self maps\mp\survival\_trigger::setCustomHintString(&"SENTRY_CANNOT_PLACE");
			}
		}
...
onSentryDeath()
{
	...
	else self.owner maps\mp\survival\_trigger::clearCustomHintString();
}
...
trophyUseListener(owner)
{
	...
	trigger = maps\mp\survival\_trigger::createTrigger("trophy_pickup", self.origin, 0, 32, 32, &"MP_PICKUP_TROPHY");
	trigger.trophy = self;	
