...
{	
	...
	level.shopsZone = [];	//[weapon_origin, weapon_angles, equipment_origin, equipment_angles, support_origin, support_angles]
	level.shopsZone["mp_dome"] = [(75, -365, -376), 24, (-1478, 1087, -413), -66, (435, 2470, -240), -95];
	
	map = getDvar("mapname");	
	if (isDefined(level.shopsZone[map]))
	{
		zones = level.shopsZone[map];
...
spawnShop(origin, yrot, type)
{
	...
	shopIcon = newHudElem();
	shopIcon setShader(wayPoint, 12, 12);
	shopIcon setWaypoint(true, true);
	shopIcon SetTargetEnt(shopModel[0]);
	
	maps\mp\survival\_trigger::createTrigger(type + "_shop", origin, 0, 55, 55, "Hold ^3[{+activate}] ^7to use " + hintString);
}
