...
test()
{
	...
	trigger = maps\mp\survival\_trigger::createTrigger("test_trigger", self.origin, 0, 55, 55, "test trigger");
	trigger maps\mp\survival\_trigger::setUseHold(1500, &"MP_CAPTURING_CRATE", 1);
	
	for (;;)
	{
		self waittill("trigger_use_holded", trigger);
...
{
	self clearLastStand();
	if (isDefined(self.currMenu)) self maps\mp\dynamic_menu_utility::closeDynamicMenu();
	self hideArmorHud();
...
triggerUseHandle()
{
	...
	for (;;)
	{
		...
		{			
			self maps\mp\dynamic_menu_utility::openDynamicMenu(trigger.tag);
			self loadItemCost();
		...
		if (self.isCarryObject || self hasStreak()) continue;
...
hudInit()
{
	...
	self maps\mp\survival\_trigger::clearCustomHintString();
}
